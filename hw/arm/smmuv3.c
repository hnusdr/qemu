                        ...
                        SMMUEventInfo *event, SMMUTransCfg *cfg)
{
    ...
    AddressSpace *as = (cfg->sec_sid == SMMU_SEC_SID_S)
                        ? cfg->s_as : cfg->ns_as;
    /* TODO: guarantee 64-bit single-copy atomicity */
    ret = dma_memory_read(as, addr, buf, sizeof(*buf), cfg->txattrs);
    if (ret != MEMTX_OK) {
        qemu_log_mask(LOG_GUEST_ERROR,
                         ...
                         SMMUEventInfo *event, SMMUTransCfg *cfg)
{
    ...
    if (bank->features & SMMU_FEATURE_2LVL_STE) {
        ...
        AddressSpace *as = (cfg->sec_sid == SMMU_SEC_SID_S)
                           ? cfg->s_as : cfg->ns_as;
        /* TODO: guarantee 64-bit single-copy atomicity */
        ret = dma_memory_read(as, l1ptr, &l1std, sizeof(l1std),
                              cfg->txattrs);
        if (ret != MEMTX_OK) {
            qemu_log_mask(LOG_GUEST_ERROR,
                                       ...
                                       SMMUSecSID sec_sid)
{
    ...
    if (cfg) {
        ...
        /* Cache both address spaces at config time */
        cfg->ns_as = smmu_get_address_space(bc, SMMU_SEC_SID_NS);
        if (sec_sid > SMMU_SEC_SID_NS) {
            cfg->s_as = smmu_get_address_space(bc, SMMU_SEC_SID_S);
...
static int smmuv3_cmdq_consume(SMMUv3State *s, SMMUSecSID sec_sid)
{
    ...
    while (!smmuv3_q_empty(q)) {
        ...
        /*
         * Command with SSec == 1 on Non-secure Command queue, including Realm
         * Command queue, is not allowed. CERROR_ILL will be raised according to
