...
* ``ITD_REG_DMA_ATTRS`` (0x18): DMA attributes which shadow some fields in
  ...
  - bit[3]: space_valid (1=space is valid, 0=ignore space and default to Non-Secure)
    ``space`` field in MemTxAttrs is consumed only when ``space_valid`` is set. For Secure/Non-Secure, ``secure`` and ``space`` must match; mismatches return ``ITD_DMA_ERR_BAD_ATTRS``. Other bits are reserved but can be wired up easily if future tests need to pass extra attributes.
