 ...
 * the secure-only RAM window at 0x0e00_0000. The x86 q35 test uses 1 GiB of
 * RAM and exercises the TSEG window near the top of RAM.
 */
...
#define X86_GUEST_RAM_MBYTES    1024
#define X86_TSEG_MBYTES         1
#define TEST_TSEG_BASE ((uint64_t)(X86_GUEST_RAM_MBYTES - X86_TSEG_MBYTES) * \
                        1024 * 1024)

static void enable_tseg(QTestState *qts)
{
    QPCIBus *pcibus;
    QPCIDevice *pcidev;
    uint8_t esmramc_val;
    ...
    /* Use QEMU's tseg-blackhole. Reads return 0xff and writes are discarded. */
    ...
    esmramc_val |= MCH_HOST_BRIDGE_ESMRAMC_TSEG_SZ_1MB;
...
static void test_arm_scalar_attrs(void)
{
    ...
    assert_qtest_error(qts, "writeb 0x%" PRIx64 " 0x77 space=non-secure\n",
                       (uint64_t)(TEST_ADDR_ARM_S + 0x6));
    assert_qtest_error(qts, "readb 0x%" PRIx64 " space=non-secure\n",
                       (uint64_t)(TEST_ADDR_ARM_S + 0x6));

    assert_qtest_error(qts, "writeb 0x%" PRIx64 " 0x88\n",
                       (uint64_t)(TEST_ADDR_ARM_S + 0x7));
    assert_qtest_error(qts, "readb 0x%" PRIx64 "\n",
                       (uint64_t)(TEST_ADDR_ARM_S + 0x7));
...
static void test_x86_scalar_attrs(void)
{
    ...
    /*
     * The byte right before TSEG must remain normal RAM, while the first
     * byte in TSEG must only be reachable through the SMM address space.
     */
    g_assert_cmpuint(qtest_readb_attrs(qts, TEST_TSEG_BASE - 1, NULL), ==, 0);
    qtest_writeb_attrs(qts, TEST_TSEG_BASE - 1, 0x5a, NULL);
    g_assert_cmpuint(qtest_readb_attrs(qts, TEST_TSEG_BASE - 1, NULL), ==, 0x5a);

    val = qtest_readb_attrs(qts, TEST_TSEG_BASE, NULL);
    g_assert_cmpuint(val, ==, 0xff);

    qtest_writeb_attrs(qts, TEST_TSEG_BASE, 0x11, "secure");
    val = qtest_readb_attrs(qts, TEST_TSEG_BASE, "secure");
    g_assert_cmpuint(val, ==, 0x11);

    val = qtest_readb(qts, TEST_TSEG_BASE);
    g_assert_cmpuint(val, ==, 0xff);

    val = qtest_readb_attrs(qts, TEST_TSEG_BASE, NULL);
    g_assert_cmpuint(val, ==, 0xff);

    qtest_writeb_attrs(qts, TEST_TSEG_BASE, 0x22, NULL);
    val = qtest_readb_attrs(qts, TEST_TSEG_BASE, NULL);
    g_assert_cmpuint(val, ==, 0xff);
...
static void test_x86_bulk_attrs(void)
{
    ...
    qtest_memwrite_attrs(qts, TEST_TSEG_BASE + 0x100,
                         wbuf, sizeof(wbuf), "secure");
    qtest_memread_attrs(qts, TEST_TSEG_BASE + 0x100,
                        rbuf, sizeof(rbuf), "secure");
    g_assert(memcmp(wbuf, rbuf, sizeof(wbuf)) == 0);

    qtest_memread_attrs(qts, TEST_TSEG_BASE + 0x100,
                        rbuf, sizeof(rbuf), NULL);
    g_assert(memcmp(rbuf, ffbuf, sizeof(rbuf)) == 0);

    qtest_memset_attrs(qts, TEST_TSEG_BASE + 0x100,
                       0x5a, sizeof(rbuf), NULL);
    qtest_memread_attrs(qts, TEST_TSEG_BASE + 0x100,
                        rbuf, sizeof(rbuf), "secure");
    g_assert(memcmp(wbuf, rbuf, sizeof(wbuf)) == 0);
    ...
    qtest_bufwrite_attrs(qts, TEST_TSEG_BASE + 0x200,
                         wbuf, sizeof(wbuf), "secure");
    qtest_bufread_attrs(qts, TEST_TSEG_BASE + 0x200,
                        rbuf, sizeof(rbuf), "secure");
    g_assert(memcmp(wbuf, rbuf, sizeof(wbuf)) == 0);
