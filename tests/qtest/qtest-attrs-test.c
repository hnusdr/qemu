 ...
 *
 * This test verifies that the qtest *_secure and *_space commands work correctly.
 *
 ...
 *     without it cpu_get_address_space(cpu, 1) returns NULL and the test aborts.
 *
 * - x86 (pc machine, TCG):
 ...
 *
 * Default RAM size for both "virt" machine in ARM and "pc" machine in x86 is 128 MiB.
 * We define a 4 KiB size offset above the RAM base, both in ARM and x86, as the test
 * address.
 */
...
static void test_arm_all_spaces(void)
{
    QTestState *qts;
    uint32_t val;
    ...
    /*
     * Test all 4 ARM security spaces via *_space commands.
     * SECURE is auto-derived from SPACE using arm_space_is_secure():
     *   Secure(0)    -> secure=1  (ARM Secure AS)
     *   NonSecure(1) -> secure=0  (normal AS)
     *   Root(2)      -> secure=1  (ARM Secure AS, highest privilege)
     *   Realm(3)     -> secure=0  (normal AS, CCA realm)
     */
    qts = qtest_init(ARM_MACHINE_ARGS);

    /* Secure (space=0) -> secure=1 */
    qtest_writel_space(qts, TEST_ADDR_ARM, 0x00000000, ARMSS_Secure);
    val = qtest_readl_space(qts, TEST_ADDR_ARM, ARMSS_Secure);
    g_assert_cmpuint(val, ==, 0x00000000);

    /* NonSecure (space=1) -> secure=0 */
    qtest_writel_space(qts, TEST_ADDR_ARM + 0x100, 0x11111111, ARMSS_NonSecure);
    val = qtest_readl_space(qts, TEST_ADDR_ARM + 0x100, ARMSS_NonSecure);
    g_assert_cmpuint(val, ==, 0x11111111);

    /* Root (space=2) -> secure=1 */
    qtest_writel_space(qts, TEST_ADDR_ARM + 0x200, 0x22222222, ARMSS_Root);
    val = qtest_readl_space(qts, TEST_ADDR_ARM + 0x200, ARMSS_Root);
    g_assert_cmpuint(val, ==, 0x22222222);

    /* Realm (space=3) -> secure=0 */
    qtest_writel_space(qts, TEST_ADDR_ARM + 0x300, 0x33333333, ARMSS_Realm);
    val = qtest_readl_space(qts, TEST_ADDR_ARM + 0x300, ARMSS_Realm);
    g_assert_cmpuint(val, ==, 0x33333333);
...
int main(int argc, char **argv)
{
    ...
    qtest_add_func("/qtest/arm/secure/writeb_readb", test_arm_writeb_readb_secure);
    qtest_add_func("/qtest/arm/secure/writew_readw", test_arm_writew_readw_secure);
    qtest_add_func("/qtest/arm/secure/writel_readl", test_arm_writel_readl_secure);
    qtest_add_func("/qtest/arm/secure/writeq_readq", test_arm_writeq_readq_secure);
    qtest_add_func("/qtest/arm/secure/memwrite_memread", test_arm_memwrite_memread_secure);
    qtest_add_func("/qtest/arm/secure/memset", test_arm_memset_secure);

    /* ARM *_space tests (Secure/NonSecure/Root/Realm, requires secure=on) */
    qtest_add_func("/qtest/arm/space/writeb_readb", test_arm_writeb_readb_space);
    qtest_add_func("/qtest/arm/space/memwrite_memread", test_arm_memwrite_memread_space);
    qtest_add_func("/qtest/arm/space/memset", test_arm_memset_space);
    qtest_add_func("/qtest/arm/space/all_spaces", test_arm_all_spaces);
    qtest_add_func("/qtest/arm/secure/single_shortcuts", test_arm_single_secure);
    qtest_add_func("/qtest/arm/space/single_shortcuts", test_arm_single_space);

    /* x86 *_secure tests (SMM address space, X86ASIdx_SMM = 1) */
    qtest_add_func("/qtest/x86/secure/writeb_readb", test_x86_writeb_readb_secure);
    qtest_add_func("/qtest/x86/secure/writew_readw", test_x86_writew_readw_secure);
    qtest_add_func("/qtest/x86/secure/writel_readl", test_x86_writel_readl_secure);
    qtest_add_func("/qtest/x86/secure/writeq_readq", test_x86_writeq_readq_secure);
    qtest_add_func("/qtest/x86/secure/memwrite_memread", test_x86_memwrite_memread_secure);
    qtest_add_func("/qtest/x86/secure/memset", test_x86_memset_secure);
