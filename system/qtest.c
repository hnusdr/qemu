 ...
 * security state (e.g., ARM TrustZone/CCA).
 *
 ...
 * ARM (Secure state, ARMASIdx_S); all other targets always use 0 for the asidx parameter.
 * When issuing *_secure commands, we assert that the secure AddressSpace must exist.
 *
 ...
 *
 * Secure access (*_secure) commands (x86/ARM):
 * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
 *
 * .. code-block:: none
 *
 ...
 *  < OK
 *
 * Space access (*_space) commands (ARM-specific):
 * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
 *
 * .. code-block:: none
 *
...
static void qtest_process_command(CharFrontend *chr, gchar **words)
{
    ...
    if (strcmp(words[0], "irq_intercept_out") == 0
         ...
         * Only specifies SECURE parameter, SPACE defaults based on SECURE.
         */
        ...
        fprintf(stderr, "[qtest] %s addr=0x%" PRIx64 " value=0x%" PRIx64
                       " secure=%" PRIu64 "\n",
                       words[0], addr, value, secure);

        attrs.secure = secure & 1;
        // attrs.space = secure ? ARMSS_Secure : ARMSS_NonSecure;
        ...
        if (attrs.secure) {
            ...
            g_assert(as);
            printf("[qtest] using secure AddressSpace\n");
            /* TODO:  这里修改成 qtest_send(chr, "ERR *_space commands are ARM-specific\n");的方式？ */
        } else {
            as = first_cpu->as;
        }
        ...
        fprintf(stderr, "[qtest] %s addr=0x%" PRIx64 " secure=%" PRIu64 "\n",
                       words[0], addr, secure);
        ...
        fprintf(stderr, "[qtest] %s addr=0x%" PRIx64 " len=0x%" PRIx64
                       " secure=%" PRIu64 "\n",
                       words[0], addr, len, secure);
        ...
        fprintf(stderr, "[qtest] %s addr=0x%" PRIx64 " len=0x%" PRIx64
                       " pattern=0x%lx secure=%" PRIu64 "\n",
                       words[0], addr, len, pattern, secure);
        ...
        fprintf(stderr, "[qtest] %s addr=0x%" PRIx64 " value=0x%" PRIx64
                       " space=%" PRIu64 "\n",
                       words[0], addr, value, space);

        attrs.space = space & 3;
        attrs.secure = arm_space_is_secure(attrs.space);
        attrs.unspecified = false;
        ...
        fprintf(stderr, "[qtest] %s addr=0x%" PRIx64 " space=%" PRIu64 "\n",
                       words[0], addr, space);
        ...
        fprintf(stderr, "[qtest] %s addr=0x%" PRIx64 " len=0x%" PRIx64
                       " pattern=0x%lx space=%" PRIu64 "\n",
                       words[0], addr, len, pattern, space);
